...
public:
	...
	const static unsigned MAX_TIMERS = 10;
	...
	// get how milliseconds left to the end of timer
	unsigned long leftTime(unsigned timerId);

	// destroy the specified timer
	void deleteTimer(unsigned timerId);

	// restart the specified timer
	void restartTimer(unsigned timerId);

	// returns true if the specified timer is enabled
	boolean isEnabled(unsigned timerId);

	// enables the specified timer
	void enable(unsigned timerId);

	// disables the specified timer
	void disable(unsigned timerId);

	// enables the specified timer if it's currently disabled,
	// and vice-versa
	void toggle(unsigned timerId);
...
private:
	...
	typedef struct 
	{
	  ...
	  byte toBeCalled;					// deferred function call (sort of) - N.B.: only used in run()
	} timer_t;
